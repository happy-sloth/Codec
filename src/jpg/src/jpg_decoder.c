...
typedef ret_type_t (*chunk_handler)(jpg_chunk*, jpg_header_t *p_data);

static ret_type_t signature_handler(jpg_chunk* chunk, jpg_header_t *p_data);
static ret_type_t comment_handler(jpg_chunk* chunk, jpg_header_t *p_data);
static ret_type_t quant_table_handler(jpg_chunk* chunk, jpg_header_t *p_data);
                                    ...
                                    jpg_header_t *p_data)
{
    ...
    for (int index = 0; index < NUM_OF_MARKERS; index++)
    {
        ...
        {
            ret_val = handler_map[index].handler(&b_chunk, p_data);
            return ret_val;
        }
